...
// ==========================================
// 🌟 核心元件：雙重人格智慧對話框 (採用 Gemini 2.5 Flash 穩定版)
// ==========================================
const SmartMotivator = ({ data, targetName, allStoresData }) => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'model', text: '老闆好！我是您的 AI 營運特助 Manie 🤖\n我已經載入 Gemini 2.5 最強引擎，隨時為您分析全區戰況。想了解哪間分店的異常，或誰的潛力最大呢？' }
  ]);
  ...
  const checkAvailableModels = async () => {
    if (!apiKey) return;
    setIsTyping(true);
    setTypingStatus('正在診斷 API 支援清單...');
    setChatHistory(prev => [...prev, { role: 'user', text: '🔎 正在診斷 API 支援模型清單...' }]);
    ...
    try {
      ...
      if (resData.models) {
        const modelNames = resData.models.map(m => m.name.replace('models/', '')).join(', ');
        setChatHistory(prev => [...prev, { role: 'model', text: `✅ 查詢成功！您的金鑰支援：\n\n${modelNames}\n\n建議優先使用 **gemini-2.5-flash**。` }]);
      } else {
        throw new Error(resData.error?.message || '無法取得模型列表');
      }
    } catch (error) {
      setChatHistory(prev => [...prev, { role: 'model', text: `❌ 查詢失敗。原因：${error.message}` }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    const baseStatus = detectedStore ? `正在調閱 ${detectedStore} 的報表資料...` : '正在掃描全區業績異常點...';
    setTypingStatus(baseStatus);

    // 設定定時切換狀態文字，營造處理感
    const statusInterval = setInterval(() => {
        setTypingStatus(prev => {
            if (prev.includes('調閱') || prev.includes('掃描')) return '數據大量比對分析中...';
            if (prev.includes('分析')) return '正在探詢最佳營運建議方式...';
            return '即時策略擬定中...';
        });
    }, 2500);
    ...
    if (!apiKey) {
      ...
      setTimeout(() => {
        setChatHistory(prev => [...prev, { role: 'model', text: '⚠️ 系統提醒：您目前尚未填寫 Gemini API Key。' }]);
        setIsTyping(false);
      }, 800);
      return;
    }
...
${JSON.stringify(simplifiedData)}`;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`[HTTP ${res.status}] ${errorData.error?.message || 'API 請求失敗'}`);
      }

      const resData = await res.json();
      const responseText = resData.candidates?.[0]?.content?.parts?.[0]?.text;
      
      setChatHistory(prev => [...prev, { role: 'model', text: responseText || '抱歉，分析引擎回傳了空值。' }]);
    } catch (error) {
      setChatHistory(prev => [...prev, { role: 'model', text: `⚠️ 連線錯誤：${error.message}\n建議點擊上方 (i) 圖示檢查您的 API 權限狀態。` }]);
    } finally {
      clearInterval(statusInterval);
      setIsTyping(false);
    }
          ...
          <div className="bg-gradient-to-r from-indigo-800 via-indigo-600 to-blue-500 p-5 flex justify-between items-center text-white flex-shrink-0 shadow-lg relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] opacity-80 mt-1 uppercase font-black tracking-widest text-indigo-100">Thinking Engine 2.5 Flash</p>
              </div>
            </div>
